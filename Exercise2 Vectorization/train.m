...
%display_network(images(:, 1:100));先不显示减小内存
%disp(labels(1:10));
...
[opttheta, cost] = minFunc( @(p) sparseAutoencoderCost(p, ...
 ...
 [opttheta, cost] = minFunc( @(p) sparseAutoencoderCost(p, ...
   ...
   [opttheta, cost] = minFunc( @(p) sparseAutoencoderCost(p, ...
                              ...
                              opttheta, options);
                          disp('8 over')     
% as it will be"out of memory"error in matlab,so we trained the nets for 8 times,in each time the number of iterations is 50 
