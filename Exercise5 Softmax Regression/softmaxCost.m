...
p = exp(M) ./ repmat(sum(exp(M)), numClasses, 1);%we can use p=bsx(@rdivide,exp(M),sum(exp(M))), book p70
cost = -(1. / numCases) * sum(sum(groundTruth .* log(p))) + (lambda / 2.) * sum(sum(theta.^2));%p64
thetagrad = -(1. / numCases) * (groundTruth - p) * data' + lambda * theta;%p64
